		...
		t.Execute(w, func() map[string]string {
			if len(projectName) > 0 && len(serviceName) > 0 && len(jsonParsed) > 0 && json.Valid([]byte(jsonParsed)) {
				...
				return map[string]string{
					"Main":           templates.GenerateMain(serviceName, projectName),
					"MainPath":       fmt.Sprintf("/%s/main.go", utils.ToLower(projectName)),
					"ModelPath":      fmt.Sprintf("/%s/internal/models/%s.go", utils.ToLower(projectName), utils.ToLower(serviceName)),
					"Model":          model,
					"ControllerPath": fmt.Sprintf("/%s/internal/controllers/%s_controller.go", utils.ToLower(projectName), utils.ToLower(serviceName)),
					"Controller":     templates.GenerateController(serviceName, projectName),
					"ServicePath":    fmt.Sprintf("/%s/internal/services/%s_service.go", utils.ToLower(projectName), utils.ToLower(serviceName)),
					"Service":        templates.GenerateServices(serviceName, projectName),
					"RepositoryPath": fmt.Sprintf("/%s/internal/repositories/%s_repository.go", utils.ToLower(projectName), utils.ToLower(serviceName)),
